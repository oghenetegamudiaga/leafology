...
import { z } from 'zod';
import { Leaf, UserPlus, ArrowRight, ShieldAlert, Lock, Mail, User } from 'lucide-react';
...
export const RegisterPage: React.FC = () => {
  ...
  const onSubmit = (data: RegisterFormValues) => {
    registerAuth(data.name, data.email);
    navigate('/account');
  };
    ...
    <div className="bg-[#FAF8F5] text-[#241C15] py-16 px-4 sm:px-6 lg:px-12 min-h-[75vh] flex items-center justify-center">
      <div className="w-full max-w-md bg-white rounded-3xl p-8 border border-stone-200/80 shadow-xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="font-serif text-3xl font-bold text-[#1A331E]">Create Your Account</h1>
          <p className="text-xs text-stone-500 font-light">
            Join Leafology to track orders, manage subscriptions & save wishlist items
          </p>
        </div>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full pl-10 pr-3 py-3 text-xs rounded-xl border bg-[#FAF8F5] focus:bg-white focus:outline-none transition-all ${
                  errors.name ? 'border-red-500 focus:ring-1 focus:ring-red-500' : 'border-stone-200 focus:border-[#1A331E]'
                }`}
              />
        ...
        <div className="pt-4 border-t border-stone-100 text-center text-xs text-stone-600">
          ...
          <Link to="/login" className="font-bold text-[#1A331E] hover:underline">
            Log in
          </Link>
        </div>
